    ...
    <>
      ...
      {items.length >= 20 && (
        <div className="mt-8 flex justify-center gap-4">
          {page > 1 && (
            <a
              href={`/komik/pustaka?page=${page - 1}`}
...
export default async function KomikPustakaPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold">Pustaka Komik</h1>
          <p className="text-muted-foreground">Koleksi pustaka komik lengkap</p>
        </div>
      </div>

      <Suspense fallback={<GridSkeleton count={12} />}>
        <PustakaResults page={page} />
      </Suspense>
    </div>
  );
}
